...
cp /mnt/postgresql.conf /var/lib/pgpro/1c-12/data/ 					# перемещение нового файла конфигураций
#
#
...
rm -rf /mnt/*-nls_*.deb									# удаление ненужных пакетов
#
...
#
systemctl enable srv1cv83 		                				# добавление серсиса в автозагрузку
systemctl start srv1cv83						                  # запуск сервиса 1С
#
...
#
dpkg -i haspd_8.53-eter1ubuntu_amd64.deb						# установка пакетов
#
systemctl enable haspd									# добавление сервиса HASP в автозагрузку
systemctl start haspd									# запуск сервиса HASP
#
rm -rf haspd_8.53-eter1ubuntu_amd64.deb							# удаление пакетов
#
#
# Установка Apache2
#
